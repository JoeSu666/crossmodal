...
from model.mil import MILModel
from utils.utils import get_attribute, calculate_cv_statistics, save_cv_results, print_cv_results
...
def create_experiment_name(config, model_name, ckpt_path, fold=None, tag=None):
...
def train_single_fold(config, model_name, ckpt_path, fold, tag, output_dir='./outputs'):
    """Train a single fold of the cross-validation"""
    
    # Create experiment name for this fold
    experiment_name = create_experiment_name(config, model_name, ckpt_path, fold, tag)
    
    print(f"\n{'='*60}")
    print(f"Training Fold {fold}: {experiment_name}")
    print(f"Model: {model_name}")
    print(f"Checkpoint: {ckpt_path if ckpt_path else 'None (training from scratch)'}")
    print(f"{'='*60}")
    ...
    model = MILModel(
        ...
        patch_encoder_ckpt_path=ckpt_path,
        learning_rate_1=config['model']['learning_rate_1'],
        learning_rate_2=config['model']['learning_rate_2'],
        weight_decay=config['model']['weight_decay'],
        scheduler=config['model']['scheduler'],
        max_epochs=config['training']['max_epochs'],
        class_names=config['class_names'],
        freeze_block=config['model']['freeze_block'],
        warmup_enabled=config['model'].get('warmup_enabled', False),
        warmup_epochs=config['model'].get('warmup_epochs', 0)
    )
    ...
    # Close wandb run if using wandb to avoid conflicts
    if config['logging']['use_wandb']:
        wandb.finish()
...
def main():
    ...
    parser.add_argument('--model_name', type=str, choices=["univ2_distill", "univ2_distill_head"], 
                        help='Override model name from config')
    parser.add_argument('--fold', type=int, default=None, help='index of fold to run')
    ...
    # Determine model name and checkpoint path
    model_name = args.model_name or config['model']['model_name']
    
    ckpt_path = config['model']['patch_encoder_ckpt_path']

    
    # Run experiment(s)
    # Single fold experiment
    test_results, best_ckpt = train_single_fold(config, model_name, ckpt_path, fold, args.tag, args.output_dir)
